...
setosa_weight = setosa_df['PetalWidthCm'].tolist()
versicolor_length = versicolor_df['PetalLengthCm'].tolist()
versicolor_weight = versicolor_df['PetalWidthCm'].tolist()
...
plt.scatter(setosa_length,setosa_weight)
plt.scatter(versicolor_length,versicolor_weight)
plt.xlabel('꽃잎 길이')
plt.ylabel('꽃잎 너비')
plt.show()
...
weight = setosa_weight + versicolor_weight

iris_data = [[l,w] for l , w in zip(length,weight)]

print(setosa_df.info) # 50
print(versicolor_df.info) # 50
