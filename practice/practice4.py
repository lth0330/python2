...
print(knr.score(test_input,test_target))   # 0.6035558780841
...
print(lr.predict([[4**2,4]]))
print(lr.predict([[6**2,6]]))
...
print(lr.score(train_poly, train_target)) # 0.54446623377852
plt.show()

plt.scatter(test_input,test_target)
plt.plot(point, lr.predict(point_poly))
print(lr.score(test_poly, test_target)) # 0.5413125141187343
plt.show()

# [단계 9] 단순 선형 회귀(1차 방정식)와 비교했을 때, 특성을 제곱한 다항 회귀(2차 방정식)가 가지는 수학적/표현적 장점을 주석으로 서술하시오.

# 일차방정식은 직선 , 2차 방정식 곡선이다. 
